...
import { Unlock, Lock, List, Droplet, Dumbbell, Flower2, Leaf, Target, Brain, Pencil, Pill, User } from 'lucide-react';
...
function Routines() {
  ...
  const getIconForHabit = (text) => {
    const t = text.toLowerCase();
    if (t.includes('water') || t.includes('drink')) return <Droplet size={18} strokeWidth={1.5} />;
    if (t.includes('exercise') || t.includes('workout') || t.includes('gym')) return <Dumbbell size={18} strokeWidth={1.5} />;
    if (t.includes('meditate') || t.includes('yoga') || t.includes('breath')) return <Flower2 size={18} strokeWidth={1.5} />;
    if (t.includes('junk') || t.includes('diet') || t.includes('food') || t.includes('eat')) return <Leaf size={18} strokeWidth={1.5} />;
    if (t.includes('focus') || t.includes('work') || t.includes('study')) return <Target size={18} strokeWidth={1.5} />;
    if (t.includes('mind') || t.includes('read') || t.includes('learn')) return <Brain size={18} strokeWidth={1.5} />;
    if (t.includes('create') || t.includes('write') || t.includes('draw')) return <Pencil size={18} strokeWidth={1.5} />;
    if (t.includes('pill') || t.includes('supplement') || t.includes('vitamin')) return <Pill size={18} strokeWidth={1.5} />;
      ...
      if (r.id === routineId) {
        ...
        if (!newHistory[dateStr]) {
          ...
          if (dateStr === todayDateStr) {
             ...
             addPoints((basePoints / 2) + (basePoints * streakMultiplier));
             if (updateQuest) updateQuest('routine', 1);
          }
        } else if (newHistory[dateStr] === true || newHistory[dateStr] === 'rest') {
          delete newHistory[dateStr];
          if (dateStr === todayDateStr) {
             addPoints(-100); 
          }
  ...
  const gridTemplateColumns = `minmax(340px, 1fr) ${columnWidthsNumeric.map(w => w + 'px').join(' ')} 50vw`;
  ...
  useEffect(() => {
    // Small delay to ensure layout is computed before scrolling
    setTimeout(() => {
            ...
            <EditableWidget id="routines-interactive-charts" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', flex: 2 }}>
              ...
              <div style={{ backgroundColor: 'var(--card-bg)', padding: '2rem', border: '1px solid var(--border-color)', borderRadius: '16px', boxShadow: '0 8px 32px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ position: 'relative', width: '100%', height: '180px' }}>
                  ...
                  <div style={{ position: 'absolute', left: 0, top: 0, width: '50px', height: '160px', backgroundColor: 'var(--card-bg)', zIndex: 10, display: 'flex', alignItems: 'center' }}>
                  ...
                  <div 
                    ...
                    style={{ width: '100%', height: '180px', overflowX: 'auto', overflowY: 'hidden', paddingLeft: '50px' }}
                  >
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                        <YAxis 
                          ...
                          width={50}
                        />
                        <Line dataKey={`metrics.${activeChartMetric}`} hide={true} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                    ...
                    <div style={{ width: `${scrollableDays.length * 45}px`, height: '160px' }}>
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Line 
                          ...
                          name={activeMetricObj ? activeMetricObj.label : 'Metric'} 
          ...
          <AnimatePresence>
            {visibleRoutines.map((routine) => (
              ...
              >
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ color: 'var(--text-color)' }}>
                      {getIconForHabit(routine.text)}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      <span style={{ fontSize: '1rem', fontWeight: 'bold', color: 'var(--text-color)', fontFamily: 'var(--font-sans)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        {routine.text}
                      </span>
                      <span style={{ fontSize: '0.65rem', color: 'var(--dim-text)', marginTop: '0.15rem' }}>
                        {Object.values(routine.history || {}).filter(v => v === true).length} / {monthDays.length} days
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    >
                      {routine.isLocked ? <Lock size={14} /> : <Unlock size={14} />}
                    </button>
                ...
                {monthDays.map((dateObj, i) => {
                  const state = routine.history ? routine.history[dateObj.dateStr] : null;
                  ...
                  return (
                    ...
                    >
                      {isCompleted && (
                        <div style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: 'var(--accent-green)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000' }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        </div>
                      )}
                      {isPartial && (
                        <svg width="20" height="20" viewBox="0 0 24 24">
                          <circle cx="12" cy="12" r="9" fill="transparent" stroke="var(--accent-green)" strokeWidth="2"/>
                          <path d="M12 3 a9 9 0 0 1 0 18 z" fill="var(--accent-green)"/>
                        </svg>
                      )}
                      {!isCompleted && !isPartial && !isRest && !isInaccessible && (
                        <svg width="20" height="20" viewBox="0 0 24 24">
                          <circle cx="12" cy="12" r="9" fill="transparent" stroke="var(--border-color)" strokeWidth="2"/>
                        </svg>
                      )}
                      {isInaccessible && !isCompleted && !isPartial && (
                        <svg width="20" height="20" viewBox="0 0 24 24">
                          <circle cx="12" cy="12" r="9" fill="transparent" stroke="var(--border-color)" strokeWidth="2" opacity="0.3"/>
                        </svg>
                      )}
                    </div>
                  );
